...
-- 3. Look for those movies with the lowest and highest rental count and the respective revenue exluding zero inventory
...
-- 4. Look for the most popular film category with respective revenue and the rental duration
SELECT c.category_id, c.name, COUNT(r.rental_id) AS rental_count, SUM(p.amount) AS revenue, f.rental_duration
FROM category c
...
JOIN rental r ON i.inventory_id = r.inventory_id
JOIN payment p ON r.rental_id = p.rental_id
GROUP BY c.category_id, c.name, f.rental_duration
ORDER BY rental_count DESC;



-- 5. Look at the rental count of customers and their location
...
--- 9.1 Too look at those film with 'Boring' in description and comepare its rental duration with the average rental duration of the same category
